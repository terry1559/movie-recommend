...
function openBrowser(url) {
  ...
  if (!urlPattern.test(url)) { log(`Error: URL does not match expected Vercel OAuth pattern: ${url}`); return; }
...
async function waitForAuthUrl() {
  for (let i = 0; i < 40; i++) {
    ...
    } catch (e) { if (e.code !== 'ENOENT') { log(`Warning: Error reading log file: ${e.code || e.message}`); } }
  }
  return null;
}
async function doLogin() {
  ...
  if (authUrl) {
    log(''); log('========================================'); log('Authorization URL extracted'); log('========================================'); log('');
    openBrowser(authUrl);
...
async function main() {
  ...
  if (checkLoginStatus()) { log(''); log('Already logged in'); console.log(JSON.stringify({ status: 'already_logged_in' })); process.exit(0); }
  await doLogin();
}
main();
